...
import { migrateAnonymousData } from './api';
import {
  getAnonSessionIds,
  getAnonProfileIds,
  setLocalProfiles,
...
export async function claimAnonymousData(
  ...
  const sessionIds = getAnonSessionIds();
  const profileIds = getAnonProfileIds();
  ...
  try {
    ...
    // Importing dynamically to avoid circular deps and keep this util lightweight.
    try {
      const { getKundliProfiles } = await import('./api');
